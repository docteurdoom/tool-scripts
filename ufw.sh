...
LANS="192.168.100.228 10.0.0.228"
...
if [ $ON -gt 1 ]; then
		...
		ufw allow out on ${INTERFACE} from any to any &> /dev/null || die "Failed to set rule" ; eend $?
		ebegin "Allowing in on ${INTERFACE} from any to any"
		ufw allow in on ${INTERFACE} from any to any &> /dev/null || die "Failed to set rule" ; eend $?
